...
dir2 <- "C:/Users/ThinkPad/Desktop/Lucky/ScoreEB/Frontiers_in_Genetics_Resubmit/Data/Sample_Size_Data/mrMLM_Format/2000/Pheno_2000/"
...
write.table(kin, paste0(dir1, "Kinship_2000.txt"), sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)

file_pheno <- paste0(dir2, "Sim.Pheno.10QTNs.2000.csv")
pheno <- fread(file_pheno)
new_pheno <- cbind(sim_fam[,1], pheno)
colnames(new_pheno) <- c("<Phenotype>", colnames(pheno))

write.table(new_pheno,paste0(dir1, "mrMLM_Phenotype_2000.csv"), sep = ",", quote = FALSE, row.names = FALSE, col.names = TRUE)
